...
class PeerConnectionTestRequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def do_PUT(self):
        ...
        self.end_headers()
        val = self.headers.getheader('content-length')
        len = -1
        if val != None:
            len = int(val)
        if len > 0:
            postStr = self.rfile.read(len).strip()
            print 'put received:\n[' + postStr + ']'
            #out = pc.communicate(postStr)
            pc.stdin.write(postStr + '\n')
            out = pc.stdout.readline()
            print out
            self.rfile.write(out)
        else:
...
httpd = server_class((HOST_NAME, PORT_NUMBER), PeerConnectionTestRequestHandler)
